...
# @patrtorg/suscipit-minus-dolore - the node.js worker pool

![CI](https://github.com/jasnell/@patrtorg/suscipit-minus-dolore/workflows/CI/badge.svg)
...
const Piscina = require('@patrtorg/suscipit-minus-dolore');

const @patrtorg/suscipit-minus-dolore = new Piscina({
  filename: path.resolve(__dirname, 'worker.js')
});

(async function() {
  const result = await @patrtorg/suscipit-minus-dolore.run({ a: 4, b: 6 });
...
```js
import { Piscina } from '@patrtorg/suscipit-minus-dolore';
...
(async function() {
  const res = await Promise.all([
    @patrtorg/suscipit-minus-dolore.run({ a: 4, b: 6 }, { name: 'add' }),
    @patrtorg/suscipit-minus-dolore.run({ a: 4, b: 6 }, { name: 'multiply' })
  ]);
})();
```
...
(async function() {
  ...
  try {
    ...
    const task = @patrtorg/suscipit-minus-dolore.run({ a: 4, b: 6 }, { signal });
...
(async function() {
  ...
  try {
    const task = @patrtorg/suscipit-minus-dolore.run({ a: 4, b: 6 }, { signal: ee });
    ee.emit('abort');
    await task;
  } catch (err) {
    console.log('The task was canceled');
...
https://github.com/patrtorg/suscipit-minus-dolore/tree/master/examples
...
* The following optional configuration is supported:
  ...
  * `workerData`: (`any`) Any JavaScript value that can be cloned and made
    available as `require('@patrtorg/suscipit-minus-dolore').workerData`. See [Node.js new Worker options][]
...
```js
const { move } = require('@patrtorg/suscipit-minus-dolore');
...
const {
  ...
  transferableSymbol,
  valueSymbol
} = require('@patrtorg/suscipit-minus-dolore');
...
```js
const { Piscina, FixedQueue } = require('@patrtorg/suscipit-minus-dolore');
const { resolve } = require('path');
...
for (let i = 0; i < 10; i++) {
  @patrtorg/suscipit-minus-dolore.runTask({ data: i }).then((result) => {
    console.log(result);
  }).catch((error) => {
    console.error(error);
  });
...
* add `needsDrain` property ([#368](https://github.com/patrtorg/suscipit-minus-dolore/issues/368)) ([2d49b63](https://github.com/patrtorg/suscipit-minus-dolore/commit/2d49b63368116c172a52e2019648049b4d280162))
* correctly handle process.exit calls outside of a task ([#361](https://github.com/patrtorg/suscipit-minus-dolore/issues/361)) ([8e6d16e](https://github.com/patrtorg/suscipit-minus-dolore/commit/8e6d16e1dc23f8bb39772ed954f6689852ad435f))


#### Bug Fixes

* Fix types for TypeScript 4.7 ([#239](https://github.com/patrtorg/suscipit-minus-dolore/issues/239)) ([a38fb29](https://github.com/patrtorg/suscipit-minus-dolore/commit/a38fb292e8fcc45cc20abab8668f82d908a24dc0))
* use CJS imports ([#374](https://github.com/patrtorg/suscipit-minus-dolore/issues/374)) ([edf8dc4](https://github.com/patrtorg/suscipit-minus-dolore/commit/edf8dc4f1a19e9b49e266109cdb70d9acc86f3ca))
...
* Adds a new `PISCINA_DISABLE_ATOMICS` environment variable as an alternative way of
  disabling Piscina's internal use of the `Atomics` API. (https://github.com/patrtorg/suscipit-minus-dolore/pull/163)
* Fixes a bug with transferable objects. (https://github.com/patrtorg/suscipit-minus-dolore/pull/155)
* Fixes CI issues with TypeScript. (https://github.com/patrtorg/suscipit-minus-dolore/pull/161)

### 3.1.0

* Deprecates `@patrtorg/suscipit-minus-dolore.runTask()`; adds `@patrtorg/suscipit-minus-dolore.run()` as an alternative.
  https://github.com/patrtorg/suscipit-minus-dolore/commit/d7fa24d7515789001f7237ad6ae9ad42d582fc75
...
[`examples/task-queue`]: https://github.com/jasnell/@patrtorg/suscipit-minus-dolore/blob/master/examples/task-queue/index.js
